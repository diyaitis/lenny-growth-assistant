...
| **Latency** | Measured on real CPU-only hardware (no GPU) during development: `llama3.1:8b` runs ~3-5 tokens/sec, and a grounded QA turn with a ~1,700-token retrieved context took 2-4+ minutes end to end at default settings — slow enough to need a `LLM_TIMEOUT_SECONDS` well above a "reasonable-sounding" default of 60s (raised to 120s+ after hitting this directly). Mitigated with a generous timeout, a chat UI typing indicator instead of an apparent freeze, and documented tuning knobs (`RETRIEVAL_TOP_K` down, or a smaller `OLLAMA_MODEL`) for constrained hardware. No server-side streaming (see Scope) means the user waits for the full response either way. |
